from helper import simulate_move

def minimax(state, depth, maximizing, visited_nodes = None):
    if (visited_nodes is None):
        visited_nodes = set()
        
    if state["number"] >= 1200 or depth < 0:
        return state["player_score"], None, visited_nodes
    visited_nodes.add((state["number"], depth))
    
    best_move = None
    if maximizing:
        max_eval = float('-inf')
        for move in [2, 3, 4]:
            new_state = simulate_move(state, move)
            eval_score, _, _ = minimax(new_state, depth - 1, False, visited_nodes)
            if eval_score > max_eval:
                max_eval = eval_score
                best_move = move
        return max_eval, best_move, visited_nodes
    else:
        min_eval = float('inf')
        for move in [2, 3, 4]:
            new_state = simulate_move(state, move)
            eval_score, _, _ = minimax(new_state, depth - 1, True, visited_nodes)
            if eval_score < min_eval:
                min_eval = eval_score
                best_move = move
        return min_eval, best_move, visited_nodes